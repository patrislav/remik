...
function sockets(server) {
  ...
  io.on('connection', async (socket) => {
    ...
    async function emitRoomUsers(target, room) {
      try {
        let users = await User.find({ realm, id: { $in: room.users } }).exec();
        (target || io).emit('room.users', room.id, users, []);
      }
      catch(e) {
        console.log(e);
      }
    ...
    async function emitRooms(target) {
      try {
        let rooms = (await Room.find({ realm }).exec()).map(room => room.toJSON());
